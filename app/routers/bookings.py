...
_booking_state_lock = threading.RLock()
...
def _next_unique_reference_code(db: Session) -> str:
    while True:
        candidate = reference.next_reference_code()
        exists = db.query(Booking.id).filter(Booking.reference_code == candidate).first()
        if exists is None:
            return candidate
...
def cancel_booking(
    ...
    with _booking_state_lock:
        ...
        booking_date = booking.start_time.date().isoformat()
    ...
    cache.invalidate_report(user.org_id)
    cache.invalidate_availability(room_id, booking_date)
